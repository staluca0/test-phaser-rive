import { Artboard, CanvasRenderer, RiveCanvas, StateMachineInstance, SMIInput, Fit, Alignment, LinearAnimationInstance, StateMachine, LinearAnimation } from "@rive-app/canvas-advanced-single";
...
export class RiveObject extends Phaser.GameObjects.Extern {
  ...
  constructor(scene: Phaser.Scene, key: string, x?: number, y?: number, artboard?: string, stateMachine?: string) {
    ...
    if (factory) {
      ...
      console.log('RiveObject creato con artboard e stateMachine:', this.artboard, this.stateMachine);
    } else {
    ...
    this.rive = rive;
    console.log('RiveObject configurato:', this);
  }
  ...
  private setStateMachineByName(stateMachineName?: string): void {
    if (!this.artboard) {
      return;
    }
  ...
  play(animationName?: string): void {
    ...
    let animation: LinearAnimation | undefined;
    if (!animationName) {
    ...
    console.log('play completato con successo.');
  ...
  preUpdate(time: number, delta: number): void {
    ...
    // console.log('preUpdate chiamato con delta:', this.delta);
  ...
  renderCanvas(renderer: Phaser.Renderer.Canvas.CanvasRenderer, camera: Phaser.Cameras.Scene2D.Camera, calcMatrix: Phaser.Math.Matrix4): void {
        console.log('renderCanvas chiamato ');
